...
import Logo from '../../../components/logos/logo-40px'
  ...
  return (
    <Popover
      className={` fixed top-0 z-50 w-full  ${
        !atTopOfPage ? ' bg-primary-600 shadow-md' : null
      }`}
    >
            ...
            .map((item, i) => {
              return (
                <Link href={item.href} key={i}>
                  <a className="hidden text-base font-medium text-white hover:text-primary-200 md:block">
                    {item.title}
                  </a>
                </Link>
              )
          ...
          <div className="items-center justify-end hidden md:flex md:flex-1 lg:w-0">
            {navSections
              ...
              .map((item, i) => {
                return (
                  <Link href={item.href} key={i}>
                    <a className="inline-flex items-center justify-center px-4 py-2 ml-8 text-base font-medium text-white border border-transparent rounded-md shadow-sm whitespace-nowrap bg-primary-800 hover:bg-primary-900">
                      {item.title}
                    </a>
                  </Link>
                )
              ...
              <div>
                {navSections
                  ...
                  .map((item, i) => {
                    return (
                      <Link href={item.href} key={i}>
                        <a className="inline-flex items-center justify-center w-full px-4 py-2 text-base font-medium text-white border border-transparent rounded-md shadow-sm whitespace-nowrap bg-primary-800 hover:bg-primary-900">
                          {item.title}
                        </a>
                      </Link>
                    )
